"""
SPiKE model for robot trajectory prediction.
"""
...
class SPiKERobot(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_head = self._build_mlp_head(dim, mlp_dim, num_robot_coords, dropout2)
    ...
    def _build_mlp_head(self, dim, mlp_dim, num_robot_coords, dropout):
        return nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, mlp_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(mlp_dim, num_robot_coords),
        )

    def forward(self, x):  # [B, L, N, 3]
        """
        Forward pass of the SPiKE robot trajectory model.
        """
        device = x.device
        ...
        robot_coords = self.mlp_head(output)
