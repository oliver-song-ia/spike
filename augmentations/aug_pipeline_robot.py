"""
Augmentation pipeline for robot trajectory data.
"""
...
from augmentations.mirror_aug_robot import MirrorAugRobot
from augmentations.center_aug import CenterAug


class AugPipelineRobot:
    """Class to represent an augmentation pipeline for robot trajectory data."""

    def __init__(self):
        """Constructor."""
        # Include both original and robot-specific augmentations
        self.aug_classes_ = {sub.__name__: sub for sub in Augmentation.__subclasses__()}
        # Add robot-specific mirror augmentation
        self.aug_classes_['MirrorAugRobot'] = MirrorAugRobot
        self.aug_classes_['MirrorAug'] = MirrorAugRobot  # Map MirrorAug to robot version
    ...
    def create_pipeline(self, p_dict_list):
        ...
        """
        self.pipeline_ = []
        for aug_dict in p_dict_list:
            aug_name = aug_dict["name"]
            # Use robot-specific mirror augmentation
            if aug_name == "MirrorAug":
                aug_name = "MirrorAugRobot"

            aug_class = self.aug_classes_[aug_name]
            self.pipeline_.append(aug_class(**aug_dict))

        self.probabilities = [torch.rand(1).item() for _ in self.pipeline_]

    def augment(self, clip, gt, target_frame="last"):
        """
        Augment a sequence of tensors with robot trajectory ground truth.

        Args:
            clip (torch.Tensor): Input point cloud sequence with dimensions [frames_per_clip, n_points, 3].
            gt (torch.Tensor): Ground truth robot coordinates with dimensions [1, 4, 3].
        ...
        Returns:
            ...
            aug_gt (torch.Tensor): Augmented ground truth with dimensions [4, 3].
        """
        ...
        for cur_aug in enumerate(self.pipeline_):
            ...
            if isinstance(cur_aug_object, MirrorAugRobot):
        ...
        # Convert robot trajectory GT to proper format
        aug_gt_tensor = (
